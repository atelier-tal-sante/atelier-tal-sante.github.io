---
layout: page
permalink: /cfp/
title: Call for Papers
description: Information about submitting to the TAL@Santé 2026 workshop. 
---

#### Key Dates 

* Call for papers announcement: 7 mars 2026
* Date limite de soumission : 29 avril 2026 (23:59 AoE)
* Notification aux auteurs : 13 mai 2026
* Versions définitives : 21 mai 2026
* Workshop: 29 juin 2026

#### Type of Contributions

* Summary articles (3 pages max + references):
  * Preliminary work in progress
  * Description of a research project
  * Translation of a recently accepted (or pending) article in an international conference
* Standard articles (between 6 and 10 pages + references):
  * New contribution
  * State of the art
  * A negative result that provides a new perspective on a scientific problem
  * Position statement presenting a viewpoint on the state of research in NLP and health

#### Submission Instructions

* Submissions should be made in PDF format through OpenReview: [https://openreview.net/group?id=ls2n.fr/CORIA-TALN/2026/Workshop/TAL-Sante](https://openreview.net/group?id=ls2n.fr/CORIA-TALN/2026/Workshop/TAL-Sante).
* Submissions should be anonymized and follow the [CORIA-TALN 2026 format](https://coria-taln-2026.ls2n.fr/soumission/).
* Reviewing is done double-blind.
* The official language of the conference is French. If all authors are French-speaking, the papers must be written in French. If one of the authors is not French-speaking, the papers may be written in English.

Note: Creating an account on OpenReview can take up to two weeks due to moderation. New users are therefore encouraged to register in advance.
